...
Lerim is a local-first SQLite CLI/runtime. These targets describe the intended
operating envelope for one developer machine, not a hosted multi-tenant service.

## Baseline assumptions

- Storage is on a local SSD, not a network filesystem.
- One background runtime owns routine sync and maintenance.
...
| Area | Target |
| --- | --- |
| `ask` retrieval latency | p50 under 500 ms, p95 under 2 s for a warm local index up to 100k records/project. |
| `ask` end-to-end latency | p95 under 8 s for normal questions when the answer model is available locally or through a responsive API. |
| Sync throughput | Drain normal agent-session traces at 5-20 MB/minute, bounded mostly by extraction/model time rather than SQLite writes. |
| Queue recovery | After a crash or restart, queued sync work resumes without manual cleanup; duplicate processing must be idempotent. |
...
| Maintenance | Routine compaction, supersession, and index refresh should finish in minutes for an active project, not block reads. |

## Search index freshness contract

The `records` table is the source of truth. `records_fts` and
`record_embeddings` are derived indexes used by `ask` retrieval. Search quality
...
- Records per project: design for 10k-100k active records/project before users
  need to archive, split projects, or tune maintenance.
...
- Allow many readers, but assume only one active writer for `sync` or
  `maintain` at a time.
...
- p95 `ask` retrieval above 2 s after index tuning at 100k+ active
  records/project
- sustained sync backlog that cannot drain overnight on a normal developer
  laptop
...
maintenance, and queue visibility over introducing a server.
