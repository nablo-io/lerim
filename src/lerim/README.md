...
Current architecture uses DSPy pipelines for trace ingestion, context
...
- `agents/`: agent flows (`trace_ingestion/`, `context_curator/`, `context_graph/`, `context_answerer/`, `context_brief/`) and typed contracts (`contracts.py`)
...
6. `agents/trace_ingestion/` and `agents/context_curator/` for the write-side DSPy flows.
7. `agents/context_graph/` for the derived link graph over curated records.
8. `agents/context_answerer/` for the read-only DSPy answer flow.
9. `agents/context_brief/` for DSPy context-brief compilation.
