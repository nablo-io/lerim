"""Clustered LangGraph maintain pipeline backed by BAML."""
...
from lerim.agents.maintain.operations import (
    apply_maintain_action_plans,
    summarize_application,
)
from lerim.agents.maintain.state import MaintainGraphState
...
def run_maintain_graph(
    *,
    ...
    """Run the BAML maintain graph and return its final state."""
    graph = build_maintain_graph(
...
def build_maintain_graph(
    *,
    ...
    """Compile the inventory, review, and mutation maintain graph."""
    ...
    def load_inventory(state: MaintainGraphState) -> dict[str, Any]:
        ...
        if progress:
            print(f"  maintain inventory active_records={len(records)}", flush=True)
        return {
            "records": records,
    ...
    def build_clusters(state: MaintainGraphState) -> dict[str, Any]:
        ...
        if progress:
            print(
                f"  maintain clusters={len(clusters)}",
                flush=True,
            )
        return {
            "clusters": clusters,
    ...
    def review_clusters(state: MaintainGraphState) -> dict[str, Any]:
        """Ask BAML to review each semantic cluster."""
        llm_calls = int(state.get("llm_calls") or 0)
        observations: list[dict[str, Any]] = []
        plans: list[Any] = []
        for cluster in state.get("clusters") or []:
            if llm_calls >= max_llm_calls:
                raise RuntimeError(f"BAML maintain exceeded max_llm_calls={max_llm_calls}.")
            if progress:
                print(f"  maintain review cluster {cluster.get('cluster_id')}", flush=True)
            result, retry_observations, attempts = _call_baml_with_retries(
                lambda instruction, cluster=cluster: baml_runtime.ReviewMaintainCluster(
    ...
    def review_health(state: MaintainGraphState) -> dict[str, Any]:
        """Ask BAML to review singleton record health batches."""
        llm_calls = int(state.get("llm_calls") or 0)
        ...
        for index, batch in enumerate(health_batches, start=1):
            ...
            if llm_calls >= max_llm_calls:
                raise RuntimeError(f"BAML maintain exceeded max_llm_calls={max_llm_calls}.")
            if progress:
                print(f"  maintain review health batch {index}", flush=True)
            result, retry_observations, attempts = _call_baml_with_retries(
                lambda instruction, batch=batch, index=index: baml_runtime.ReviewMaintainHealthBatch(
                    run_instruction=instruction,
    ...
    def apply_actions(state: MaintainGraphState) -> dict[str, Any]:
        """Validate and apply proposed maintain actions."""
        evidence_record_ids = {
            str(record_id)
        ...
        if progress:
            print(f"  maintain apply actions={summary.applied_actions}", flush=True)
        return {
    ...
    graph = StateGraph(MaintainGraphState)
...
def _run_instruction() -> str:
    """Return maintain task framing for BAML calls."""
    return (
    ...
    while True:
        ...
        try:
            ...
            if not _is_recoverable_baml_error(exc) or attempts > MAX_BAML_MODEL_RETRIES:
                raise
            if progress:
                print(f"  maintain retry {stage} attempt={attempts}", flush=True)
            observations.append(
                _observation(
                    "model_retry",
                    False,
...
def _semantic_retry_observation(validation_error: str) -> str:
    ...
    return (
        ...
        "maintain action plan was unsafe or incomplete. Retry with a complete safe "
