"""Record inventory and semantic grouping helpers for maintain."""
...
MAINTAIN_RECORD_LIMIT = 200
MAINTAIN_NEIGHBOR_LIMIT = 4
MAINTAIN_MAX_CLUSTER_SIZE = 8
MAINTAIN_HEALTH_BATCH_SIZE = 16
MAINTAIN_TEXT_LIMIT = 1800
MAINTAIN_PREVIEW_LIMIT = 520
MAINTAIN_RECORD_FIELDS = (
    "record_id",
    "kind",
    "title",
    "body",
...
def prepare_maintain_store(
    *,
    ...
    """Initialize store provenance for one maintain run."""
...
def load_active_records(
    *,
    context_db_path: Path,
    project_identity: ProjectIdentity,
    limit: int = MAINTAIN_RECORD_LIMIT,
) -> list[dict[str, Any]]:
    """Load a bounded active-record inventory for maintain."""
    store = ContextStore(context_db_path)
    store.initialize()
    store.register_project(project_identity)
    listing = store.query(
...
def build_similarity_clusters(
    *,
    ...
    neighbor_limit: int = MAINTAIN_NEIGHBOR_LIMIT,
    max_cluster_size: int = MAINTAIN_MAX_CLUSTER_SIZE,
) -> list[dict[str, Any]]:
...
def build_health_batches(
    *,
    ...
    batch_size: int = MAINTAIN_HEALTH_BATCH_SIZE,
...
def record_search_query(record: dict[str, Any]) -> str:
    ...
    return "\n".join(parts)[:MAINTAIN_TEXT_LIMIT]
...
def _compact_record(record: dict[str, Any], *, preview: bool) -> dict[str, Any]:
    ...
    for field in MAINTAIN_RECORD_FIELDS:
        value = record.get(field)
        if value is None:
            continue
        if isinstance(value, str):
            text_limit = MAINTAIN_PREVIEW_LIMIT if preview and field == "body" else MAINTAIN_TEXT_LIMIT
