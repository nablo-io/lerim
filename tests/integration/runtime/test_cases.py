...
def test_sync_artifact_paths_are_stable_per_flow(monkeypatch, live_config, live_repo_root):
	"""Sync writes artifacts only under the canonical sync workspace layout."""
	expectation = load_runtime_expectation("sync_artifact_paths_are_stable_per_flow")["expected"]
	...
	assert run_folder.parent == workspace_root / str(expectation["workspace_subdir"])
	assert result["workspace_root"] == str(workspace_root)
	assert set(result["artifacts"]) == set(expectation["artifact_names"])
	assert (run_folder / "agent.log").read_text(encoding="utf-8").strip() == "sync complete"
...
def test_ask_debug_trace_preserves_ordered_tool_flow(monkeypatch, live_config, live_repo_root):
	"""Ask debug payload keeps message order and tool flow intact."""
	expectation = load_runtime_expectation("ask_debug_trace_preserves_ordered_tool_flow")["expected"]
	ctx = build_runtime_case_context(
		monkeypatch=monkeypatch,
		live_config=live_config,
		live_repo_root=live_repo_root,
	...
	monkeypatch.setattr(
		...
		lambda **kwargs: (AskResult(answer="Final answer"), build_ordered_ask_messages()),
	)
	...
	assert debug["messages"][5]["parts"][0]["content"] == "The latest change updated the recent record."


def test_maintain_change_counts_reflect_real_mutations(monkeypatch, live_config, live_repo_root):
	"""Maintain payload counts should match the actual store mutations from the run."""
	expectation = load_runtime_expectation("maintain_change_counts_reflect_real_mutations")["expected"]
	ctx = build_runtime_case_context(
		monkeypatch=monkeypatch,
		live_config=live_config,
		live_repo_root=live_repo_root,
	...
	def _fake_run_maintain(**kwargs):
		...
		store.update_record(
			...
			changes={"body": "Retries in the worker loop should stay bounded and observable."},
	...
	result = ctx.runtime.maintain(repo_root=live_repo_root, session_id="runtime-maintain-case")

	run_folder = Path(result["run_folder"])
	workspace_root = live_config.global_data_dir / "workspace"
	assert run_folder.parent == workspace_root / str(expectation["workspace_subdir"])
	assert result["artifacts"] == {
		"agent_log": str(run_folder / "agent.log"),
		"subagents_log": str(run_folder / "subagents.log"),
	}
	...
	assert (run_folder / "agent.log").read_text(encoding="utf-8").strip() == "maintain complete"
	archived_episode = ctx.store.fetch_record(str(seed_episode["record_id"]), project_ids=[ctx.project_id], include_versions=True)
	assert archived_episode is not None
	assert archived_episode["status"] == "archived"
	updated_fact = ctx.store.fetch_record(str(seed_fact["record_id"]), project_ids=[ctx.project_id], include_versions=True)
	assert updated_fact is not None
	assert updated_fact["body"] == "Retries in the worker loop should stay bounded and observable."


def test_sync_retries_transient_error_and_then_writes_artifacts(monkeypatch, live_config, live_repo_root):
	"""Sync should retry one transient failure and still finish with normal artifacts."""
	expectation = load_runtime_expectation("sync_retries_transient_error_and_then_writes_artifacts")["expected"]
	ctx = build_runtime_case_context(
		monkeypatch=monkeypatch,
		live_config=live_config,
		live_repo_root=live_repo_root,
	...
	assert run_folder.parent == live_config.global_data_dir / "workspace" / str(expectation["workspace_subdir"])
	assert (run_folder / "agent.log").read_text(encoding="utf-8").strip() == "sync recovered"
	assert (run_folder / "session.log").exists()
	assert (run_folder / "agent_trace.json").exists()


def test_runtime_sync_then_maintain_then_ask_with_real_artifacts(monkeypatch, live_config, live_repo_root):
	"""A sync-created record should survive maintain and be what ask later answers from."""
	expectation = load_runtime_expectation("runtime_sync_then_maintain_then_ask_with_real_artifacts")["expected"]
	ctx = build_runtime_case_context(
		monkeypatch=monkeypatch,
		live_config=live_config,
		live_repo_root=live_repo_root,
	...
	sync_result = ctx.runtime.sync(trace_path=trace_path, session_id="runtime-sync-chain")
	maintain_result = ctx.runtime.maintain(repo_root=live_repo_root, session_id="runtime-maintain-chain")
	...
	assert sync_run_folder.parent == live_config.global_data_dir / "workspace" / str(expectation["sync_workspace_subdir"])
	assert maintain_run_folder.parent == live_config.global_data_dir / "workspace" / str(expectation["maintain_workspace_subdir"])
	assert sync_run_folder.exists()
	assert maintain_run_folder.exists()
	assert (sync_run_folder / "agent.log").read_text(encoding="utf-8").strip() == "sync wrote initial fact"
	assert (maintain_run_folder / "agent.log").read_text(encoding="utf-8").strip() == "maintain strengthened fact"
	assert cost == 0.0
	...
	assert debug is not None
	assert [item["tool_name"] for item in debug["tool_calls"]] == expectation["ask_tool_call_order"]
