...
@pytest.mark.llm
@pytest.mark.agent
def test_maintain_duplicate_durable_records_superseded(
    live_config,
    live_repo_root,
) -> None:
    """Maintain should keep the stronger duplicate active and supersede the weaker one."""
    expectation = load_maintain_expectation("duplicate_durable_records_superseded")["expected"]
    ...
    with connect_context_db(live_config.context_db_path) as conn:
        archive_count = int(
            conn.execute(
                ...
                ("integration-maintain-duplicate_durable_records_superseded",),
            ).fetchone()[0]
        )
...
@pytest.mark.llm
@pytest.mark.agent
def test_maintain_fresh_duplicate_should_not_be_archived(
    live_config,
    live_repo_root,
) -> None:
    """Maintain should supersede fresh duplicate durable records instead of archiving them."""
    expectation = load_maintain_expectation("fresh_duplicate_should_not_be_archived")["expected"]
    outcome = run_maintain_case(
    ...
    with connect_context_db(live_config.context_db_path) as conn:
        archive_count = int(
            conn.execute(
                "SELECT COUNT(*) FROM record_versions WHERE changed_by_session_id = ? AND change_kind = 'archive'",
                ("integration-maintain-fresh_duplicate_should_not_be_archived",),
            ).fetchone()[0]
        )
...
@pytest.mark.llm
@pytest.mark.agent
def test_maintain_obsolete_fact_replaced_by_new_truth(
    live_config,
    live_repo_root,
) -> None:
    """Maintain should supersede older truth when a newer active fact replaces it."""
    expectation = load_maintain_expectation("obsolete_fact_replaced_by_new_truth")["expected"]
    outcome = run_maintain_case(
    ...
    with connect_context_db(live_config.context_db_path) as conn:
        archive_count = int(
            conn.execute(
                "SELECT COUNT(*) FROM record_versions WHERE changed_by_session_id = ? AND change_kind = 'archive'",
                ("integration-maintain-obsolete_fact_replaced_by_new_truth",),
            ).fetchone()[0]
        )
...
@pytest.mark.llm
@pytest.mark.agent
def test_maintain_no_change_when_store_is_already_clean(
    live_config,
    live_repo_root,
) -> None:
    """Maintain should leave a healthy active store unchanged."""
    expectation = load_maintain_expectation("no_change_when_store_is_already_clean")["expected"]
    outcome = run_maintain_case(
...
@pytest.mark.llm
@pytest.mark.agent
def test_maintain_healthy_fresh_record_is_true_noop(
    live_config,
    live_repo_root,
) -> None:
    """A fresh healthy durable record should not be rewritten just for style."""
    expectation = load_maintain_expectation("healthy_fresh_record_is_true_noop")["expected"]
    outcome = run_maintain_case(
...
@pytest.mark.llm
@pytest.mark.agent
def test_maintain_obsolete_low_value_durable_archived(
    live_config,
    live_repo_root,
) -> None:
    """An old low-value non-episode durable row can be archived directly."""
    expectation = load_maintain_expectation("obsolete_low_value_durable_archived")["expected"]
    outcome = run_maintain_case(
...
@pytest.mark.llm
@pytest.mark.agent
def test_maintain_semantic_duplicate_found_via_search(
    live_config,
    live_repo_root,
) -> None:
    """Hidden semantic duplicates should be found via search and resolved with supersede."""
    expectation = load_maintain_expectation("semantic_duplicate_found_via_search")["expected"]
    seed_records = [
        {
