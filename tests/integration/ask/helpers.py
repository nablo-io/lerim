"""Shared runner helpers for targeted ask-agent integration cases."""
...
from lerim.agents.ask import AskResult, run_ask
...
@dataclass
class AskCaseOutcome:
    """Observed result for one ask integration case."""
...
def load_ask_expectation(case_name: str) -> dict[str, Any]:
    """Load one YAML expectation file for an ask case."""
    return load_yaml_expectation(ASK_EXPECTATIONS_DIR, case_name)
...
def run_ask_case(
    *,
    ...
    """Run one named ask case against a fresh isolated live test DB."""
    case = load_ask_expectation(case_name)
    session_id = f"integration-ask-{case_name}"
    ...
    for index, seed in enumerate(case.get("seed_records", []), start=1):
        ...
        if seed_session_id not in seeded_sessions:
            seed_session(
                ...
                agent_type="integration-ask",
                source_trace_ref=f"ask-seed:{case_name}:{index}",
            )
    ...
    result, messages = retry_on_overload(
        lambda: run_ask(
            ...
            request_limit=int(case.get("request_limit", 30)),
            return_messages=True,
        )
    )
    payload = dump_messages(messages)
    return AskCaseOutcome(
        result=result,
